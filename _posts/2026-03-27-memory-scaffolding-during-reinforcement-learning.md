...
The first is *paraphrasing*: the model learns to restate scratchpad-derived learnings in its own words within its chain-of-thought before applying them. This matters because of what it does to the gradient: when the model generates its own formulation of a principle rather than copying it from context, the resulting weight updates strengthen the circuits involved in *producing* that understanding, not the circuits involved in *retrieving* it. This is the mechanism that makes study effective for human learners: restating material in your own words strengthens production over retrieval. Whether the same principle transfers to weight updates in neural networks (whether paraphrasing during training genuinely shifts gradient pressure from context-retrieval circuits to knowledge-production circuits) is the central empirical question of this proposal, not a settled claim. When the scratchpad is later removed, those are the circuits that persist.
...
It is worth noting the relationship to existing chain-of-thought and extended thinking training, which already demonstrates that reasoning traces improve model performance. Mnemonic bridging is not claiming the reasoning trace itself is novel. The contribution, if there is one, is providing structured, reflection-generated content to populate that trace: giving the model something specific and experience-derived to reason with, rather than relying on the model to independently discover effective reasoning strategies during training.
